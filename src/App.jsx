...
const imageUrls = [
  ...
  "https://ik.imagekit.io/jpc6wnnmd/1748401423893.jpg",
  "https://ik.imagekit.io/jpc6wnnmd/1748401425085.jpg"
];
...
function App() {
  ...
  useEffect(() => {
    ...
    onValue(ref(db, `games/${room}/phase`), (snapshot) => {
      setStep(snapshot.val() || "waiting");
    });

    onValue(ref(db, `games/${room}/scores`), (snap) => {
      if (snap.exists()) setScores(snap.val());
    });

    onValue(ref(db, `games/${room}/mirrorCard`), (snap) => {
      if (snap.exists()) setMirrorCard(snap.val());
    });

    onValue(ref(db, `games/${room}/association`), (snap) => {
      if (snap.exists()) setAssociation(snap.val());
    });

    onValue(ref(db, `games/${room}/choices/${player}`), (snap) => {
  ...
  return (
    <div className="game-container">
      <h1 className="title">MirrorMind</h1>

      {step === "waiting" && (
        <div className="fade-in">
          <p className="subtitle">Ожидание второго игрока...</p>
        </div>
      )}
